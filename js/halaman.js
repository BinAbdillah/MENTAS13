/* =========================================================
halaman.js — REFACTOR v3 (hardening XSS + default tema BIRU)
Karakter halaman = "Overlay" nuansa; aksen tetap tema pengurus.
========================================================= */
  ...
  host.innerHTML = `
    <div class="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
      <span class="flex items-center gap-3">
        ...
        <b class="judul-hal" style="color:var(--heading)">${esc(judul)}</b>
      </span>
      <div id="cuaca-widget"></div>
    </div>
    <div class="nav-hal-bar mx-auto max-w-6xl px-4 pb-2.5 text-sm">
      ${prev ? `<a class="nav-link" href="${prev[0]}">← ${esc(prev[1])}</a>` : '<span></span>'}
      <a class="nav-link font-bold" href="index.html">Beranda</a>
      ${next ? `<a class="nav-link" href="${next[0]}">${esc(next[1])} →</a>` : '<span></span>'}
    </div>`;
  return d;
}
