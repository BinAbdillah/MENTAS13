/* =========================================================
app.js — REFACTOR v5 (BERSIH: muatData tunggal, onValue tunggal)
========================================================= */
...
function renderPinRonda(d) {
  ...
  host.innerHTML = `<div class="min-w-0"> <span class="block text-[11px] uppercase tracking-widest">Petugas Ronda Hari Ini</span> <b class="block">${esc(nama || ('Tim ' + tim.nama))}</b> <span class="block text-[11px]">${esc(tgl)} • giliran ${esc(tim.nama)}</span> </div>`;
}
/* ---------- BANNER (href divalidasi) ---------- */
function renderBanner(d) {
  const slot = $('#banner-slot');
  if (!slot) return;
  const b = d.banner;
  if (!bannerAktif(b)) { slot.innerHTML = ''; return; }
  const href = /^(#|https?:\/\/|[\w.-]+\.html)/.test(b.link || '') ? (b.link || '#agenda') : '#agenda';
  slot.innerHTML = `<a href="${escAttr(href)}" class="block w-full md:mx-auto md:max-w-4xl md:px-4 md:pt-4 md:pb-1" aria-label="${escAttr(b.teks)}"> <img src="${escAttr(b.gambar)}" alt="${escAttr(b.teks)}" class="h-32 w-full object-cover object-center sm:h-40 md:h-auto md:rounded-2xl md:shadow-lg" onerror="this.parentElement.remove()"> </a>`;
}
/* ---------- REVEAL & EFEK ---------- */
...
function normalisasi(d) {
  ...
  d.tema = d.tema || { preset: 'biru', custom: {} };
  d.ronda = d.ronda || {};
...
/* ---------- MUAT DATA (SATU jalur, tanpa kode menggantung) ---------- */
async function muatData() {
  const isPreview = new URLSearchParams(window.location.search).get('preview') === '1';
  if (isPreview) {
    const p = localStorage.getItem('rw13_preview_data');
    if (p) { try { return rapikan(JSON.parse(p)); } catch (e) { console.warn('Preview gagal diurai → pakai data publik.', e); } }
  }
  if (db) {
    try {
      const snap = await _fb.get(_fb.ref(db, 'data'));
      if (snap.val()) return rapikan(snap.val());
...
(async function main() {
  try {
    ...
    renderSemua(DATA);
    /* SATU subscription real-time (dengan toast) */
    if (!isPreview && db) {
      _fb.onValue(_fb.ref(db, 'data'), (snap) => {
        ...
        window._lastJson = s;
        console.log('🔄 Data Firebase berubah — merender ulang.');
        renderSemua(v);
        const t = document.createElement('div');
        t.textContent = '🔄 Data diperbarui';
        t.style.cssText = 'position:fixed;right:16px;top:80px;z-index:95;background:var(--accent);color:var(--on-accent);padding:.5rem 1rem;border-radius:999px;font-size:.85rem;font-weight:700;box-shadow:0 4px 12px rgb(0 0 0/.3)';
