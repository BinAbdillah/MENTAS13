/* =========================================================
cuaca-widget.js — REFACTOR v2 (hardening XSS)
...
(function () {
  ...
  async function isi(host) {
    try {
      ...
      host.innerHTML = `
      <button class="cuaca-widget" id="cwBtn" aria-expanded="false" title="Detail cuaca ${escAttr(kota)}">
        <span aria-hidden="true">${ikon}</span><b>${Math.round(c.temperature_2m)}°C</b>
      </button>
      <div class="cuaca-panel" id="cwPanel" role="dialog" aria-label="Detail cuaca ${escAttr(kota)}">
        <b class="block text-sm" style="color:var(--heading)">${ikon} ${esc(teks)} — ${esc(kota)}</b>
