/* =========================================================
   render.js — FINAL v3
   header: logo + moto AMANAH saja; hero tanpa CTA & marquee
   ========================================================= */
  ...
  /* ---------- HEADER: logo + moto AMANAH ---------- */
  function renderHeader(d) {
    const i = d.identitas;
    $('#header').innerHTML = `
      <div class="mx-auto flex max-w-6xl items-center gap-4 px-4 py-4 md:gap-6">
        ${renderLogo(i.logo, 'h-16 w-16 md:h-24 md:w-24')}
  ...
  /* ---------- HERO: tanpa tombol struktur/kontak, tanpa marquee ---------- */
  function renderHero(d) {
    ...
    $('#hero').innerHTML = `
      <section class="relative overflow-hidden bg-slate-900 text-white">
        ...
        <div class="relative mx-auto max-w-6xl px-4 pb-24 pt-20 md:pb-28 md:pt-28">
        ...
        <div class="grid gap-10 lg:grid-cols-[.9fr_1.1fr] lg:gap-16">
          ...
          <div class="space-y-10">
            <div class="reveal grid grid-cols-2 gap-8">
              ${[['👥', agg ? agg.jiwa : w.penduduk, 'Penduduk (jiwa)'], ['🏠', agg ? agg.kk : 0, 'Kepala Keluarga'],
                 ['📐', w.luasM2, 'Luas (m²)'], ['🚩', w.jumlahRT, 'Rukun Tetangga']]
                .map(([ik, v, l]) => `
  ...
  function renderStruktur(d) {
    ...
    const peranRT = (p) => {
      ...
      return URUTAN_RT.filter((k) => keys.includes(k))
        .concat(keys.filter((k) => !URUTAN_RT.includes(k)))
        .map((jab) => barisJabatan(jab, p[jab]));
    };
  
    $('#struktur').innerHTML = `
      <section class="bg-white py-16 md:py-24">
        <div class="mx-auto max-w-6xl px-4">
          ${judulSeksi('02', 'Struktur Organisasi', 'Penasehat, pengurus RW, pengurus RT, dan mitra')}
          ...
          <h3 class="mb-5 mt-14 text-xl font-bold text-slate-900">Bidang</h3>
          <div class="reveal grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="kartu kartu-hover p-6">
              ...
              <div class="mt-3 flex flex-wrap gap-2">
                ${(sk.anggota || []).filter((a) => ada(a)).length
                  ? (sk.anggota || []).filter((a) => ada(a)).map((a) => `<span class="rounded-full bg-emerald-600/10 px-3 py-1.5 text-sm font-medium text-emerald-700">${a}</span>`).join('')
          ...
          <div class="reveal grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="kartu p-6">
              ...
              <div class="mt-4">${peranRT(r.pengurus).join('')}</div>
            </div>`).join('')}
          </div>
          <div class="mb-5 mt-14 flex flex-wrap items-center justify-between gap-3">
            <h3 class="text-xl font-bold text-slate-900">M I T R A</h3>
          ...
          <div class="reveal grid gap-5 lg:grid-cols-3">
            ...
            <div class="kartu kartu-hover p-6">
              ...
              <div class="mt-4">${m.struktur.map((p) => barisJabatan(p.jabatan, p.nama)).join('')}</div>
            </div>`).join('')}
          </div>
        </div>
      </section>`;
    ...
    $('#fasilitas').innerHTML = `
      <section class="bg-white py-16 md:py-24">
        <div id="fasWrap" class="relative">
          <div class="mx-auto max-w-6xl px-4">${judulSeksi('04', 'Fasilitas Warga', 'diwilayah RW13')}</div>
